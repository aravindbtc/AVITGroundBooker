...
import { Building, Loader2, Save, Image as ImageIcon, IndianRupee, CreditCard } from "lucide-react";
        ...
        <Card className="w-full max-w-4xl mx-auto shadow-lg rounded-xl">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-sm text-muted-foreground">Select how users will pay for bookings. If Razorpay is selected, users must pay online. If "Without Payment" is selected, bookings are free.</p>
                    
                    <RadioGroup value={formData.paymentMethod || 'razorpay'} onValueChange={(value) => setFormData(prev => ({ ...prev, paymentMethod: value as 'razorpay' | 'without-payment' }))}>
                        <div className="flex items-center space-x-3 p-4 border border-green-200 rounded-lg hover:bg-green-50 cursor-pointer transition-colors">
                            <RadioGroupItem value="razorpay" id="razorpay" />
                            <Label htmlFor="razorpay" className="cursor-pointer flex-1 font-semibold">
                                💳 Razorpay Payment (Online Payment Required)
                                <p className="text-xs text-muted-foreground font-normal mt-1">Users must pay via Razorpay. Receipt and transaction details will be stored in their booking history.</p>
                            </Label>
                        </div>
                        <div className="flex items-center space-x-3 p-4 border border-blue-200 rounded-lg hover:bg-blue-50 cursor-pointer transition-colors">
                            <RadioGroupItem value="without-payment" id="without-payment" />
                            <Label htmlFor="without-payment" className="cursor-pointer flex-1 font-semibold">
                                🆓 Without Payment (Free Booking)
                                <p className="text-xs text-muted-foreground font-normal mt-1">Users can book for free. No payment required. Booking shown as "Without Payment" in user's booking history.</p>
                            </Label>
                        </div>
                    </RadioGroup>
                </div>
