...
export type Booking = {
  ...
  groundId?: string;
  date?: string;
  paymentMethod: 'razorpay' | 'without-payment'; // NEW: Payment method used
  payment: {
      orderId?: string;
...
export type Venue = {
  ...
  rating: number;
  images: string[];
  paymentMethod?: 'razorpay' | 'without-payment'; // NEW: Admin-selected payment method
};
...
// Serialized types for data that passes through URL params
export type SerializedSlot = Omit<Slot, 'startAt' | 'endAt'> & {
